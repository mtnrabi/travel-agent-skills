...
- **Two flight-side defects if the same audit also touches flights** (hand that work to the flight skills): `sort_type` is accepted by the schema on the one-way flight endpoint but is not forwarded, so one-way results have to be sorted client-side — it does work on round-trip; and hard routes want `use_fallback: true` (default `false`), which is slower but does better on hard routes.
