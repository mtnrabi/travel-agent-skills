...
4. **Search flights first.** The flight is the larger and least flexible number, and it sets the hotel envelope. Pass `max_price` equal to the per-person flight ceiling you are willing to spend, `currency`, `passengers[]` (1 adult, 2 child, 3 infant on lap, 4 infant in seat), and `sort_type: "Price"` — which *is* honoured on round-trip.
...
- **An empty flight result is `[]` with HTTP 200, not an error.** The flights endpoints return a bare JSON array. `[]` means "no flights on this route for these dates" — report that plainly and offer adjacent dates or a nearby airport. Do not retry the identical call; a retry is another billed request with the same answer.
...
- **`sort_type` is silently dropped on one-way.** It is accepted by the one-way schema but is not forwarded to the search, so a one-way result set is not sorted no matter what you pass. It *does* work on round-trip. If you split a trip into two one-ways (open-jaw), sort the results client-side yourself.
- **Hard routes need `use_fallback: true`.** It defaults to false. On long-haul, multi-stop or thin routes an empty or thin result set is often the fast path failing rather than genuine unavailability — retry once with `use_fallback: true` and tell the user it will be slower.
